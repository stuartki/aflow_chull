...
class QuaternaryHullRender {
  constructor(hull, showEntries, defaultColor, pointClickHandler) {
    ...
    this.gridHeight = 489.90; //wws16 300 original, 489.90 for an equilateral triangular pyramid
    this.gridMin = -1.0; //wws16 -1.0
    this.gridMax = 1.0; //wws16 1.0 
    this.triSide = 600;
    this.triMargin = 0;
  ...
  quadCoord(a,b,c,d){
    //enter percentage of each species, returns coordinates on a triangular pyramid
    let sum = 0;
    const pos = [0, 0, 0];
    const h = 1 - (d/100);

    const rshift = ((1 - h)*(this.triSide))/2;
    const upshift = ((Math.sqrt(3))*(this.triSide)/6)*(1 - h);

    const corners = [
      [this.triMargin + rshift, upshift],
      [h * this.triSide + rshift + this.triMargin, upshift],
      [(h * this.triSide / 2) + rshift + this.triMargin, h * this.triHeight + this.triMargin + upshift],
    ];
    sum = a + b + c;
    if (sum !== 0) {
      const x = a / sum;
      ...
      pos[0] = corners[0][0] * x + corners[1][0] * y + corners[2][0] * z;
      pos[1] = corners[0][1] * x + corners[1][1] * y + corners[2][1] * z;
      pos[2] = (1-h)*this.gridHeight
    }
    if (sum === 0) {
      pos[0] = rshift;
      pos[1] = upshift;
      pos[2] = this.gridHeight;
    }
    return pos;
  }
  ...
  drawGrid() {
    ...
    //wws16 drawing triangular pyramid...

    const baseGeometry = new THREE.geometry();

    baseGeometry.vertices.push(new THREE.Vector3(this.triMargin,0,0));
    baseGeometry.vertices.push(new THREE.Vector3(this.triMargin+this.triSide,0,0));
    baseGeometry.vertices.push(new THREE.Vector3(this.triMargin+ this.triSide/2, this.triMargin + this.triHeight, 0));
    baseGeometry.vertices.push(new THREE.Vector3(this.triMargin,0,0));


    const pyramidSlopeOne = new THREE.Geometry();
    const pyramidSlopeTwo = new THREE.Geometry();
    const pyramidSlopeThree = new THREE.Geometry();
    const top = this.quadCoord(0,0,0,100);

    const x = triCenter[0];
    const y = triCenter[1];
    const z = this.gridHeight;

    pyramidSlopeOne.vertices.push(new THREE.Vector3(this.triMargin,0,0));
    pyramidSlopeOne.vertices.push(new THREE.Vector3(top[0],top[1],top[2]));

    pyramidSlopeTwo.vertices.push(new THREE.Vector3(this.triMargin+this.triSide,0,0));
    pyramidSlopeTwo.vertices.push(new THREE.Vector3(top[0],top[1],top[2]));

    pyramidSlopeThree.vertices.push(new THREE.Vector3(this.triMargin+ this.triSide/2, this.triMargin + this.triHeight, 0));
    pyramidSlopeThree.vertices.push(new THREE.Vector3(top[0],top[1],top[2]));


    const gridLineBase = new THREE.Line(baseGeometry, material);

    const gridLineSideOne = new THREE.Line(pyramidSlopeOne, material);
    const gridLineSideTwo = new THREE.Line(pyramidSlopeTwo, material);
    const gridLineSideThree = new THREE.Line(pyramidSlopeThree, material);
    
    this.group.add(gridLineBase)
    ...
    gridSideTwoGeometry.vertices.push(
      new THREE.Vector3(this.triSide,0,0)
    );
    gridSideTwoGeometry.vertices.push(
      new THREE.Vector3(top[0],top[1],top[2])
    );
    ...
    gridSideThreeGeometry.vertices.push(
      new THREE.Vector3(this.triSide/2, this.triHeight, 0)
    );
    gridSideThreeGeometry.vertices.push(
      new THREE.Vector3(top[0],top[1],top[2])
    );
  ...
  drawAxis(axisMin, axisMax) {
    ...
    labelContext.fillText(
      'fourth element %', //wws16 'formation enthalpy (meV)'
      textPosX - 50,
      textPosY
    );
    ...
    this.axisName = this.createText2D('fourth element %', '#000000'); //wws16 this.axisName = this.createText2D('formation enthalpy (meV)', '#000000');
  ...
  drawHull(data) {
    ...
    this.group.remove(this.elementD); //wws16
    ...
    //wws16 start
    if (this.defaultColor){
      elemColor = '#000000' //Black...
    }

    this.elementD = this.createText2D(hullData.species[3], elemColor);
    this.elementD.position.set(this.triCenter[0],this.triCenter[1],this.gridHeight+20);
    this.group.add(this.elementD);
    ...
    for (let i = 0; i < hullData.vertices.length; i++) {
      ...
      const X = hullData.vertices[i].composition[0] * 100;
      const Y = hullData.vertices[i].composition[2] * 100;
      const Z = hullData.vertices[i].composition[1] * 100;
      const D = hullData.vertices[i].composition[3] * 100;
      const triPos = this.quadCoord(X,Y,Z,D);
  ...
  plotEntries(data) {
    ...
    for (let i = 0; i < entries.length; i++) {
      if(!(Number(entries[i].distanceToHull) === 0)){ //wws16 dont plot non-ground states
        continue;
      }
      const pX = entries[i].composition[0] * 100;
      const pY = entries[i].composition[2] * 100;
      const pZ = entries[i].composition[1] * 100;
      const pD = entries[i].composition[3] * 100; //wws16
      const pCoord = this.quadCoord(pX, pY, pZ, pD);

      const datapoint = new THREE.Vector3(
        pCoord[0],
        pCoord[1],
        pCoord[2]  //wws16 (entries[i].enthalpyFormationAtom * this.gridHeight)
      );
      let pointColor;
      if (this.defaultColor) {
  ...
  onMouseUp(event){
    event.preventDefault();
    this.updatePlottedEntries(this.hull.entries);
  }
...
export default QuaternaryHullRender;
